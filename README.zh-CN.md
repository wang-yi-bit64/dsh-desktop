...
|---------|---------------|-----------|-------------------------|-------------------------|
| `next`（默认） | npm `next` dist-tag（rc 阶段） | `0.1.5-rc.3` | `rc` | `0.7.0-rc.1` |
| `alpha` | npm `alpha` dist-tag（下一 minor 的早期预览） | `0.1.6-alpha.2` | `alpha` | `0.7.0-alpha.2` |
...
> ⚠️ **目标键告诉不了你钉的是哪个上游版本。** 目标的 `channel` 是它**对着哪条上游 dist-tag 定义**的，而 `dshVersion` 是本仓**实际钉住**的版本——两者可以不一致。截至 2026-09-24，`next` 目标钉的是 `0.1.5-rc.3`，这正是上游 **`latest`** dist-tag 指向的版本；而上游 `next` dist-tag 已前进到 `0.1.7-rc.1`（该升级另立批次：需重做 79 个补丁 hunk）。要看实时值请跑 `node scripts/dsh-targets.mjs`；**不要按目标键推断版本**。
