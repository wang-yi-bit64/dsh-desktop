...
| `npm run verify:cli-publish` | 把 `cli-publish` job 的 `run:` 段落从 `release.yml` **原文抽出并逐字执行**，跑在一次性的最小检出布局里、用一个假 `gh` 截住网络：shell 语义、glob 展开、相对路径、`gh` 参数形状。它存在的理由是：手抄一份实现去验收，验的是抄件而不是真正发布的那段。它**不**替代真发布——假 `gh` 只断言「文件存在、非空、带了 `--clobber`」。放行「本机造不出」的产物类时，判据锚在工作流的 `missing-artifact-class:` 标记上（而不是报错里的散文，散文一改豁免就静默失效），并被**产物类契约**钉住：工作流的必需清单必须 == 夹具能造的类 ∪ 本机造不出的类，清单里出现任何无出处的新类当场判红。Windows 宿主能同时造出 `.zip` 与 `.tar.gz`、也能造便携版，因此**只有它**能把上传步骤完整跑完（文件数 / `--clobber` 因此是活断言）；Linux / macOS 上造不出的那几类逐条声明，任何超出声明的缺失仍然判红。 |
