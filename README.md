...
- **Single Instance** ✅ — Second launches focus the existing window instead of duplicating processes (and un-hide it when it was parked in the tray).
- **System Tray** ✅ — The app lives in the tray, and **closing the window parks it there instead of quitting**: Harness and the LAN phone bridge keep running, which is the point of pairing your phone with a desktop that stays awake. Tray menu items reuse the application menu's **ids and handling**, so the same action cannot behave differently depending on where you clicked it, and the menu carries two live status rows (Harness phase, phone bridge state). On Windows a left click toggles the window; Linux has **no tray click events at all** (the `libappindicator` backend does not emit them), so there the menu's "Show Window" item is the way back; macOS gets a monochrome template icon so it stays legible in a dark menu bar. Quitting — from the tray, the menu or the error page — stops Harness first rather than letting the OS kill it.
  - If the tray itself cannot be created (e.g. no AppIndicator support on Linux), the shell **keeps the old behaviour** and closing the window quits: a window that hides itself with no tray to bring it back would leave the process unreachable.
- **In-App Feedback Entry** ✅ — **DSH Desktop → Send Feedback…** (also in the tray, and on the error page) opens a local feedback page that answers "what should I attach?" *before* the user leaves for GitHub: it reads the app version, the **runtime channel** (`next` / `alpha`), the bundled DSH version and the patch tally out of `MANIFEST.json`, offers the one-click redacted diagnostics export, and hands off to four whitelisted destinations (bug report form, feature request form, Discussions, upstream Harness) through the system browser. The app itself uploads nothing — you decide what to paste or attach. The channel matters because the two upstream lines ship the same desktop version numbers, so the version alone does not identify the runtime.
...
- **Headless CLI (`dsh-host-cli`) — in-repo, not a published artifact** ✅ — The same lifecycle logic the shell drives, as a standalone binary that needs no GUI and no Tauri: `start` / `stop` / `status` / `tail` / `probe` / `doctor`. It is the deliverable of **INV-6** ("the main path must be reproducible from a command line"), and it is what `scripts/smoke-launch.mjs` (L1/L2 smoke) and `scripts/fault-inject.mjs` (three-platform orphan-cleanup gate) drive, both from `target/debug/`. **It is not published as a release asset.** A publish lane existed from v0.4.0 and was retired on 2026-09-24: it had zero consumers outside this repository, and the archive was never self-sufficient anyway (it bundles no runtime, so `start` exits `3` without an assembled resource tree — point it at one with `--resource <RES>`). Retiring the lane did **not** touch the crate, the packaging script, or its falsifiable packaging assertions; only the upload step and its dry-run rehearsal were archived. Rationale and the revival trigger are in [`docs/dev-plan-cli-distribution.md`](docs/dev-plan-cli-distribution.md) §5.
...
src-tauri/
  frontend/             # Shell web pages (splash, error attribution, plugin recovery, updates, logs, feedback)
  ...
  src/                  # Tauri desktop layer (window management, application menu, IPC commands, Safe Mode, LAN mobile bridge, shell logging, updates)
    commands.rs             # The whole IPC surface: 20 commands, all returning IpcEnvelope<T>
    ...
    tray.rs                 # System tray: icon, menu (shares menu ids/handling with the app menu), close-to-tray
    feedback.rs             # Feedback page backing: runtime identity + whitelisted channel opener
...
| P4 | Unified `IpcEnvelope<T>` responses in Tauri commands; one-click redacted diagnostic exports (`diagnostics.zip`); in-app log viewer | ✅ **wired** — all commands return `IpcEnvelope<T>`; redacted export (5 rules, `app_data_dir/exports/`); `frontend/logs.html` |
...
| `npm run verify:release-workflow` | The release workflow's own silent failure modes: `tauri-action` inserts its own `build` and `--` (so `tauriScript` must not carry either), and shell variables must be `${braced}` when followed by non-ASCII punctuation (macOS bash 3.2 otherwise swallows it into the variable name). Both turned the first `v0.1.0` release fully red while every local gate stayed green. Since 2026-09-24 the CLI checks are **direction-reversed** for the retired publish lane: the `cli` / `cli-publish` jobs must be **absent**, and `gh release upload` is judged **by its object** — uploading `dist/portable/*` is allowed (the three portable assets are part of the expected 13), uploading `dist/cli/*` is red. Meanwhile the `dsh-host-cli` crate, the packaging guard, and the `target/debug/` consumers must all still be present — "cancel the publish" and "delete the crate" are different decisions, and the guard's fixtures prove both directions. |
| `npm run verify:release-assets` | **The expected release asset manifest (13 items).** Enumerates each asset **by name pattern** rather than counting: 10 from `tauri-action` (installers, `.sig` sidecars, the macOS updater bundle, `latest.json`) plus 3 portable ones. Added because the count 13 lived only in prose — a promise, not an assertion — and F13 slipped through every green gate by producing 10. Run `-- --check-release <tag>` after a publish to diff a real Release against the expected list. ⚠️ The patterns are taken from **measured** Release output, not inferred from `tauri.conf.json`: the bundler renders `DSH Desktop` as `DSH.Desktop_<ver>_x64-setup.exe` (dot-separated product, underscore-separated version) and the macOS updater bundle carries **no version segment**. A fixture feeding the hyphenated spelling must fail. |
| `npm run verify:cli-package` | The CLI packaging lane (`scripts/package-cli.mjs`): artifact names derive from `package.json` + the target triple (never hand-written), the `.sha256` sidecar is in the format `sha256sum -c` actually reads, and the archive reads back byte-identical with its executable bit intact. Also renders the release-notes table idempotently, and carries falsifiability fixtures — a tampered extraction, a missing executable bit, a mismatched sidecar and a missing archive must all go red. |
| `npm run verify:portable-package` | The portable packaging lane (`scripts/package-portable.mjs`): artifact naming, `.sha256` sidecar shape, and "extract a zip with the right command for the platform" — the publishing job runs on ubuntu while the archive under verification is a Windows-produced `.zip`, so an unconditional `spawn powershell` made that verification fail every time. The real-packaging fixtures (a missing `WebView2Loader.dll`, an undersized runtime tree) need PowerShell and therefore run on Windows only; the extraction-command rule is pure logic with the platform as a parameter, so one run covers both branches. |
