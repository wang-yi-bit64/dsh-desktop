...
- **Headless CLI (`dsh-host-cli`) as a downloadable artifact** ✅ — The same lifecycle logic the shell drives, as a standalone binary: `start` / `stop` / `status` / `tail` / `probe` / `doctor`, no GUI and no Tauri. Each release publishes per-platform archives (`.zip` for Windows, `.tar.gz` otherwise) alongside a `.sha256` sidecar and a `manifest.json`; the packaging lane verifies the archive by reading it back (byte-identical, executable bit intact) and by **executing the extracted binary** before upload, then re-verifies the downloaded copy. It **does not** bundle, download or update a runtime — point it at an assembled one with `--resource <RES>`. Published as a release asset; it is **not** inside the desktop installer. Plan and the not-yet-started runtime-bundle phase: [`docs/dev-plan-cli-distribution.md`](docs/dev-plan-cli-distribution.md).
...
| `npm run verify:release-workflow` | The release workflow's own silent failure modes: `tauri-action` inserts its own `build` and `--` (so `tauriScript` must not carry either), and shell variables must be `${braced}` when followed by non-ASCII punctuation (macOS bash 3.2 otherwise swallows it into the variable name). Both turned the first `v0.1.0` release fully red while every local gate stayed green. Also checks the CLI artifact's shape (packaging step present, `--clobber` on upload, `.sha256` sidecar uploaded, `needs: build` on the publishing job, all three platforms) and that the release-notes section is **delegated** to a tested script rather than inlined into YAML. |
...
| `npm run verify:cli-publish` | The `cli-publish` job's `run:` blocks, **executed verbatim** from `release.yml` against a throwaway checkout layout with a stub `gh` (no network): shell semantics, glob expansion, relative paths and the `gh` argument shape. It exists because a hand-copied re-implementation of those steps validated the copy rather than the thing that ships. It does **not** replace a real release — the stub only asserts "files exist, non-empty, `--clobber` passed". Where the host simply cannot produce an artifact class, the exemption keys off the workflow's `missing-artifact-class:` marker (never off the prose around it — a reworded message used to disable the exemption silently) and is pinned by an **artifact-class contract**: the workflow's required list must equal "classes the fixture produces" ∪ "classes this host cannot produce", so any new class with no source fails immediately. Windows can produce both `.zip` and `.tar.gz`, plus the Windows-only portable zip, so it is the only host that runs the upload step to completion (making the file-count / `--clobber` assertions live there); on Linux/macOS the unproducible classes are declared one by one and any *other* missing class still goes red. |
