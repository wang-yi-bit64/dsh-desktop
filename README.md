...
| `npm run verify:cli-publish` | The `cli-publish` job's `run:` blocks, **executed verbatim** from `release.yml` against a throwaway checkout layout with a stub `gh` (no network): shell semantics, glob expansion, relative paths and the `gh` argument shape. It exists because a hand-copied re-implementation of those steps validated the copy rather than the thing that ships. It does **not** replace a real release — the stub only asserts "files exist, non-empty, `--clobber` passed"; the cross-platform completeness assertion is covered by the three-OS release matrix. |
