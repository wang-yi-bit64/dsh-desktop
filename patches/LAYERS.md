...
>
> **本次升级的验证状态（2026-09-13）**：14 个补丁已在 `0.1.5-rc.1` 上通过完整验证——
> 真实组装 `14/14 applied`、三平台 CI 与 Smoke `scope=full`（真实资源树 L1 + 打包 + L2 GUI +
> 故障注入）全绿，并随 **v0.3.0** 发布。也就是说删除功能的缺席是**已在三平台确认过的现状**，
> 不是「未经检验的降级」。
