...
/**
 * 统一空白：连续空行折叠为一行空行，去除文件末尾多余空行，保留单个结尾换行，
 * 并保证**每个版本标题前恰好一个空行**。
 *
 ...
 *
 * 最后一条不变量（标题前一个空行）是 2026-09-23 补记 `0.7.0-alpha.5` 段落时
 * 暴露的**既有缺陷**的修法：覆盖路径取到的结束边界是「下一段的标题字符」，
 * 而分隔两段的空行原本属于**被覆盖的那一段**，于是替换后新段落与下一段被直接
 * 拼在一起（`…)\n## [0.1.0]`）。这种产出在编辑器与 GitHub 上**照样正常渲染**，
 * 因此它一直没被发现——文件里 `0.7.0-alpha.3` → `alpha.2` 边界就留着这个疤
 * （发布时被重写过一次，每次重写吃掉一个空行）。
 *
 * 之所以放在 `normalize()` 里而不是只修覆盖路径：这样**任何一次**生成都会把
 * 整份文件的该不变量修好，历史遗留的疤无需专项清理，也不会在下一次发布时复发。
 *
...
export function selfTest() {
  ...
  // 6d) --force 覆盖**非最后一段**时必须保住段间空行。
  //
  // 这条是真实缺陷的回归钉：覆盖路径的结束边界取在「下一段标题字符」处，而
  // 分隔两段的空行原属被覆盖的那一段，于是替换后新段落与下一段直接相连
  // （`…)\n## [0.1.0]`）。产出**照样渲染正常**，所以一直潜伏——2026-09-23
  // 补记 alpha.5 段落时才第二次踩到（文件里 alpha.3→alpha.2 边界是第一次）。
  // 断言直接盯「文件里不得存在紧贴标题的换行」，而不是盯某个内部变量：
  // 这样将来改换实现方式，只要产出仍然正确就不会误报。
  {
    const threeWay = insertSection(
      insertSection(
        insertSection('', '## [0.1.0] - 2026-01-01\n\n- a\n', '0.1.0'),
        '## [0.2.0] - 2026-02-02\n\n- b\n',
        '0.2.0',
    ...
    const repaired = insertSection(threeWay, '## [0.2.0] - 2026-02-03\n\n- b2\n', '0.2.0', true);
    check(
      !/[^\n]\n## \[/.test(repaired),
      '插入：每个版本标题前必须恰好一个空行（覆盖非末段时最易丢）',
    );
    check(repaired.includes('\n\n## [0.1.0]'), '插入：覆盖非末段后，其后一段的分隔空行必须保留');
    check(
      (repaired.match(/^## \[/gm) || []).length === 3,
      '插入：覆盖非末段不得影响段落总数',
    );
    // 反向钉：把空行去掉的输入**必须**被修正，否则上面那条断言可能因为
    // 「输入本来就有空行」而白过。这里要带上锚点——`insertSection` 找不到锚点
    // 会直接报错（那是有意为之：不猜插入位置）。
    const scarred = `${FILE_HEADER}## [0.2.0] - 2026-02-03\n\n- b2\n## [0.1.0] - 2026-01-01\n\n- a\n`;
    const healed = insertSection(scarred, '## [0.2.0] - 2026-02-04\n\n- b3\n', '0.2.0', true);
    check(!/[^\n]\n## \[/.test(healed), '插入：历史遗留的紧贴标题必须被 normalize 修正');
  }
